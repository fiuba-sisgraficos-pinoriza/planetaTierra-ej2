        ...
        void main(void) {
            ...
            float distanciaAlCentro = sqrt((uv.x-0.5)*(uv.x-0.5) + (uv.y-0.5)*(uv.y-0.5));
            float altura = (1.0 + sin(distanciaAlCentro*20.0*PI - time*20.0))*0.05/exp(distanciaAlCentro*10.0);

            position=vec3(uv.x-0.5, altura, uv.y-0.5);
